...
const GEMINI_MODEL     = process.env.GEMINI_MODEL || 'gemini-2.5-flash'; // 2.0-flash was retired (Oct 2026)
...
async function askGemini(history, ticketType) {
  ...
  // Try the configured model first, then known-good fallbacks (so a wrong/renamed model still works).
  const models = [GEMINI_MODEL, 'gemini-2.5-flash', 'gemini-flash-latest', 'gemini-2.5-flash-lite'].filter((v, i, a) => v && a.indexOf(v) === i);
  for (const model of models) {
    try {
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${GEMINI_API_KEY}`;
      const r = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body });
      if (!r.ok) { console.warn('Gemini', model, r.status, (await r.text()).slice(0, 150)); continue; }
