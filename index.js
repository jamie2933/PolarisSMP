...
async function askGemini(history, ticketType) {
  ...
  const system =
    ...
    'Keep replies warm and concise (1-3 sentences). Never invent rules, prices or punishments.\n' +
...
async function handleTicketMessage(m) {
  const ch = m.channel;
  if (isStaff(m.member)) {                          // a staff/high-staff member wrote → hand over, go quiet
...
async function postDecision(data) {
  ...
  if (Array.isArray(data.images) && data.images[0]) embed.setImage(data.images[0]);
  const row = new ActionRowBuilder().addComponents(
  ...
  if (Array.isArray(data.images) && data.images.length > 1) extra.push('🖼 ' + data.images.slice(1, 6).join('\n🖼 '));
  await channel.send({ content: extra.join('\n').slice(0, 1900) || undefined, embeds: [embed], components: [row] });
}
...
async function openMediaTicket(data) {
  ...
  }).catch((e) => { console.warn('media ticket:', e.message); return null; });
  if (!channel) return;
  const embed = new EmbedBuilder().setColor(0x9b5cff).setTitle(`🎬 Media Application #${data.id || '?'}`)
    .addFields(
      { name: 'Applicant', value: String(data.ingameName || '?'), inline: true },
  ...
  const controls = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('close').setLabel('Close').setEmoji('🔒').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('closereason').setLabel('Close with reason').setEmoji('📝').setStyle(ButtonStyle.Danger));
  await channel.send({ content: `${STAFF_ROLE_ID ? `<@&${STAFF_ROLE_ID}> ` : ''}${member ? `<@${member.id}>` : ''}`.trim() || undefined, embeds: [embed], components: [controls] });
  const lines = [];
  if (Array.isArray(data.links) && data.links.length)  lines.push('**Links:**\n🔗 ' + data.links.slice(0, 8).join('\n🔗 '));
  if (Array.isArray(data.images) && data.images.length) lines.push('**Uploads:**\n🖼 ' + data.images.slice(0, 8).join('\n🖼 '));
...
async function openReporterTicket(i, type, userId) {
  ...
  if (!channel) return i.editReply({ content: '⚠️ Could not create the ticket (check the bot\'s permissions).' });
  const controls = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('close').setLabel('Close').setEmoji('🔒').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('closereason').setLabel('Close with reason').setEmoji('📝').setStyle(ButtonStyle.Danger));
...
require('http').createServer((req, res) => {
  if (req.method === 'POST' && req.url === '/api/decision') {
    ...
    req.on('end', () => {
      ...
      handleDecisionPost(data).catch((e) => console.warn('decision post:', e.message));
