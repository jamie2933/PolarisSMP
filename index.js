...
async function openMediaTicket(data) {
  ...
  // Post each upload on its own line so Discord unfurls a preview even if the embed image was rejected.
  for (const u of imgs.slice(0, 4)) await channel.send({ content: u }).catch(() => {});
}
