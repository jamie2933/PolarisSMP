...
const OWNER_ID         = process.env.OWNER_ID || '';   // extra USER to ping on every ticket question (e.g. you)
const TICKET_PING_COOLDOWN = Math.max(0, parseInt(process.env.TICKET_PING_COOLDOWN || '30', 10)) * 1000; // re-ping throttle (s)
...
const lastTicketPing = new Map(); // channelId -> timestamp of the last staff ping (throttle)

// Staff role + owner mention used to pull a human into a ticket.
function supportPing() {
  return (STAFF_ROLE_ID ? `<@&${STAFF_ROLE_ID}> ` : '') + (OWNER_ID ? `<@${OWNER_ID}> ` : '');
}
function pingMentions() {
  const roles = STAFF_ROLE_ID ? [STAFF_ROLE_ID] : [];
  const users = OWNER_ID ? [OWNER_ID] : [];
  return { roles, users };
}
...
// First message on ticket creation — NO AI: grant staff access, post how-it-works instructions, ping staff + owner.
async function assistantOpen(channel, type, answersText, oid) {
  if (STAFF_ROLE_ID) await channel.permissionOverwrites.edit(STAFF_ROLE_ID, {
    ViewChannel: true, SendMessages: true, ReadMessageHistory: true, AttachFiles: true, ManageMessages: true,
  }).catch(() => {});
  const t = TICKETS[type] || {};
  const info = fill(t.openInfo || C.assistant.introShort, oid);
  const embed = new EmbedBuilder()
    .setColor(t.color || BRAND_COLOR)
    .setTitle('🎫 ' + (t.label || 'Ticket'))
    .setDescription(info + '\n\n' + fill(C.assistant.openFooter, oid));
  if (ASSISTANT_AVATAR) embed.setThumbnail(ASSISTANT_AVATAR);
  await channel.send({
    content: supportPing() + `<@${oid}>`,
    embeds: [embed],
    allowedMentions: { roles: pingMentions().roles, users: [...new Set([...pingMentions().users, oid])] },
  }).catch(() => {});
  lastTicketPing.set(channel.id, Date.now());
}
...
async function handleTicketMessage(m) {
  ...
  if (isHumanHandled(ch)) return;                   // a staff member replied → stay silent, they've got it
  const text = (m.content || '').toLowerCase();
  if (matchesClose(text)) return void askCloseConfirm(ch, m.author.id);

  // NO AI: the player asked something and no staff has taken over yet → ping staff + owner (throttled so
  // a burst of messages doesn't mass-ping). Every genuine new question re-pings after the cooldown.
  const last = lastTicketPing.get(ch.id) || 0;
  if (Date.now() - last < TICKET_PING_COOLDOWN) return;
  lastTicketPing.set(ch.id, Date.now());
  await ch.send({
    content: supportPing() + fill(C.assistant.stillWaiting, m.author.id),
    allowedMentions: pingMentions(),
  }).catch(() => {});
}
