...
client.on(Events.InteractionCreate, async (i) => {
  try {
    ...
    if (i.isModalSubmit() && i.customId.startsWith('modal:')) {
      ...
      // Assistant's FIRST message: jump straight to the relevant help based on the ticket answers (no welcome fluff).
      ...
      const firstIntent = matchAssistantIntent(answers.map((a) => a.value).join(' ').toLowerCase());
      const firstMsg = (firstIntent && firstIntent.type === 'reply')
        ? fill(firstIntent.text, i.user.id)
        : fill(C.assistant.introShort, i.user.id);
      await assistantSend(channel, { content: firstMsg, components: [takeover] });
...
async function handleTicketMessage(m) {
  ...
  if (intent) {
    ...
    return void assistantSend(ch, fill(intent.text, m.author.id));
  }
