...
async function handleTicketMessage(m) {
  ...
  // Gemini unavailable → try a safe rule reply, otherwise escalate to a human.
  const it = matchAssistantIntent(text);
  if (it && it.type === 'reply') return void assistantSend(ch, fill(it.text, m.author.id));
  return void escalateTicket(ch, m.author.id, it && it.reason ? it.reason : 'assistant offline');
}
