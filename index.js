...
client.on(Events.InteractionCreate, async (i) => {
  try {
    ...
    if (i.isModalSubmit() && i.customId.startsWith('modal:')) {
      ...
      // Assistant (no AI): if the answers match a website-solvable case, point them there.
      const answersText = answers.map((a) => a.value).join(' ').toLowerCase();
      const hint = (C.ticketHints || []).find((h) => h.keywords.some((k) => answersText.includes(k.toLowerCase())));
      const assistant = hint
        ? C.ticketSolvable.replace('{hint}', hint.text)
        : C.ticketDefault;
      await channel.send(fill(assistant, i.user.id)).catch(() => {});
...
// ── Chat: moderation (word filter) + FAQ auto-replies (both NO AI) ────────────
client.on(Events.MessageCreate, async (m) => {
  if (m.author.bot || !m.guild) return;
  console.log('[msg]', m.author.username, '#' + (m.channel.name || '?'), '::', JSON.stringify(m.content));
  const text = (m.content || '').toLowerCase();
  if (!text) return;

  // 1) Moderation — delete bad words + a short auto-deleting warning.
  ...
  // 2) FAQ — only short messages, first matching entry wins.
